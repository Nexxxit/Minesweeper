...
export default observer(function Leaderboard() {
  const listItems = leaderboardStore.formattedRecords.map((record, index) => (
    <li key={`${record.name}-${index}`} className="flex flex-wrap sm:flex-nowrap sm:justify-none justify-between items-center bg-yellow-400 p-5 gap-5 rounded-xl">
  ...
  return (
    <div className="flex flex-col gap-5 w-full md:w-150 lg:w-160 items-center">
      ...
      <div className="border rounded-xl p-1 w-full h-100 overflow-y-auto scrollbar-hide">
